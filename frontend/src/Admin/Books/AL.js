...
const Novel = ()=>{
    ...
    const novelHandler = async (e)=>{
        ...
        try {
            const {data} = await axios.post("http://localhost:8070/al/add" , {noteName , author , noteURL , noteImage , downloads , hearts , category} , config);
            toast("Success! Al note Added 😘")
            setLoading(false);

        } catch (error) {
            toast("Error! AL note not Added Duplicate Key Found: Book Name must be unique")
            setLoading(false);
            setIsError(true);
            setTimeout(()=>{
            ...
            <form onSubmit={novelHandler}>
                    <div className="card">
                        ...
                        <div className="card-body">
                            <center>
                                <h5 className="card-title">Add AL note</h5>
                                <TextField
